...
# ---- Virtual environment ----
	...
	@if [ -x "$(VENV_DIR)/bin/python" ] || [ -x "$(VENV_DIR)/Scripts/python.exe" ]; then \
	...
	elif [ -n "$$VIRTUAL_ENV" ] && { [ -x "$$VIRTUAL_ENV/bin/python" ] || [ -x "$$VIRTUAL_ENV/Scripts/python.exe" ]; }; then \
		...
		"$(BASE_PYTHON)" -c "import sys; required=max((3, 12), tuple(map(int, '$(PYTHON_VERSION)'.split('.')[:2]))); raise SystemExit(0 if sys.version_info[:2] >= required else 1)" || { \
			echo "ERROR: OpenMontage requires Python 3.12+ (requested: $(PYTHON_VERSION)+)."; \
	...
	@"$(RUN_PYTHON)" -c "import sys; required=max((3, 12), tuple(map(int, '$(PYTHON_VERSION)'.split('.')[:2]))); raise SystemExit(0 if sys.version_info[:2] >= required else 1)" || { \
		echo "ERROR: OpenMontage requires Python 3.12+ (requested: $(PYTHON_VERSION)+)."; \
		...
		echo "Deactivate the old environment, back it up, then recreate it with Python $(PYTHON_VERSION)+."; \
		echo "See docs/python-upgrade.md for migration and rollback instructions."; \
		exit 1; \
	}
